...
const onQueueMessage = async (message) => {
    ...
    try {
        ...
        if (stats === null) {
            ...
            await notifier.notifyErrorAll(
                message.filePath,
                helpers.errors.FILE_NOT_EXIST.code,
                helpers.errors.FILE_NOT_EXIST.message,
                taskCompleteTs,
                message.notifyChannels
            );
        ...
        try {
            ...
            _logger.error({
                step: "delete_task_from_queue",
                success: false,
                msg: "Encountered an error while attempting to delete message from queue.",
                message: message,
        ...
        _logger.error({
            message: message,
            success: false,
            msg: "Encountered an error while creating task in db.",
            error: { message: error.message, code: error.code, stack: error.stack },
        });
    }
};
...
const checkDb = async () => {
    ...
    try {
        ...
        if (currentTask) {
            ...
            _logger.debug({
                step: "throttle_poll_rate",
                msg: "Scan currently ongoing, increasing poll interval.",
                nextCheckIn: `${timeoutSeconds}ms`,
            });
        } else {
        ...
        _logger.error({
            msg: "If you're seeing this error, it means something went horribly wrong with the tasks db",
            error: { message: error.message, code: error.code, stack: error.stack },
        });
    } finally {
    ...
    try {
        for (task of scanningTasks) {
            if (Date.now() > task.scanStartTs + markStaleAfter) {
                ...
                _logger.info({
                    step: "mark_pending",
                    msg: `Task '${task.filePath}' is stale, returning it to pending state.`,
                    file: task.filePath,
                    success: true,
                });
            }
...
const prepareFileForScanning = async (task) => {
    ...
    try {
        ...
        _logger.error({
            success: false,
            msg: "Encountered an error while preparing file for scanning.",
            error: { message: error.message, code: error.code, stack: error.stack },
        });
        return null;
    }
...
const scanFile = async (file) => {
    ...
    try {
        ...
        const scanResults = await helpers.runWithTimeout(
            scanTimeout,
            scanner.scanStream(storage.getStorage().streamFile(file)),
            `Reached timeout while scanning`
        );

        if (scanResults === null) throw new Error("File could not be scanned");
...
const handleScanSuccess = async (task, scanResults) => {
    ...
    try {
        ...
        _logger.error({
            step: "mark_completed",
            success: false,
            msg: "Encountered an error while attempting to mark task as completed.",
            error: { message: error.message, code: error.code, stack: error.stack },
        });
    }
};
...
const handleScanFailure = async (task) => {
    ...
    try {
        ...
        _logger.error({
            step: "mark_failed",
            success: false,
            msg: "Encountered an error while attempting to mark task as failed.",
            error: { message: error.message, code: error.code, stack: error.stack },
        });
    }
};
